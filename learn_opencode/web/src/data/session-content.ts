...
export const SESSION_CONTENT: Record<string, SessionContent> = {
  s01: {
    ...
    learnSections: [
      {
        title: "The Problem: How Does an AI Agent Take Actions?",
        content:
      ...
      {
        title: "Real Implementation: Effect-Based Concurrency",
        content:
    ...
    designHighlights: [
      { en: "The loop checks `stop_reason != 'tool_use'` instead of checking for an explicit 'done' signal. This means the model naturally signals completion when it has nothing left to execute, with no extra protocol required.", zh: "循环通过检查 `stop_reason != 'tool_use'` 来判断完成，而非依赖显式的 'done' 信号。这意味着模型在没有更多操作可执行时会自然发出完成信号，无需额外的协议。" },
      { en: "Model selection is a configuration parameter rather than a hardcoded value. The same loop works with any OpenAI-compatible API, making it provider-agnostic from the start.", zh: "模型选择是配置参数而非硬编码值。同一个循环适用于任何兼容 OpenAI 的 API，从一开始就实现了与提供商无关。" },
    ],
  ...
  <text x="430" y="260" fill="#666" font-size="10" font-family="system-ui">Feed result back</text>
    ...
    annotatedCode: {
      overview: {
        ...
        zh: "Agent 循环是一个简洁的 while 循环，包含四个阶段：调用模型、检查工具调用、执行工具、重复。整个架构仅需约 15 行代码。",
      },
      annotations: [
        { lineStart: 1, lineEnd: 1, text: "An infinite loop: the agent continues until the model decides it is done. Each iteration represents a complete thought-action-observation cycle.", zhText: "无限循环：agent 持续运行直到模型决定完成。每次迭代都是一个完整的思考-行动-观察周期。" },
        { lineStart: 2, lineEnd: 7, text: "The LLM call. `messages` accumulates the entire conversation history, including previous tool results. `tools=TOOLS` informs the model about available actions.", zhText: "LLM 调用。`messages` 累积了包含先前工具结果在内的完整对话历史。`tools=TOOLS` 告知模型可用的操作。" },
        { lineStart: 8, lineEnd: 9, text: "The termination check: if the model did not request a tool, it is signaling completion. No special 'done' token is required.", zhText: "终止检查：如果模型没有请求工具，说明它在发出完成信号。无需特殊的 'done' 标记。" },
        { lineStart: 10, lineEnd: 12, text: "Iterating over response content blocks. Tool calls are structured objects with a name and input, the model's way of requesting the execution of a function with specific parameters.", zhText: "遍历响应内容块。工具调用是带有名称和输入的结构化对象，是模型表达'用这些参数运行此函数'的方式。" },
        { lineStart: 13, lineEnd: 17, text: "The result is appended as a user message rather than a system message. This distinction matters: the model perceives tool output as if the user provided it, preserving the natural flow of conversation.", zhText: "结果作为用户消息而非系统消息追加。这一区别很重要：模型将工具输出视为用户提供的内容，从而保持对话的自然性。" },
      ],
      flowSummary: {
        en: "Flow: LLM Call → Check stop_reason → (tool_use?) → Execute Tool → Append Result → LLM Call → (no tool_use?) → Break",
        zh: "流程：调用 LLM → 检查 stop_reason → (工具调用?) → 执行工具 → 追加结果 → 调用 LLM → (无工具调用?) → 退出",
      },
    },
    claudeCodeComparison: {
      en: "Claude Code uses the same tool-use loop architecture of send, tool_use, execute, append, and repeat. OpenCode distinguishes itself through async Effect and Stream primitives for LLM calls and a formal ToolID schema registry. Where Claude Code uses a flat tool list, OpenCode organizes tools by category (read, write, shell, skill) with structured input and output schemas.",
      zh: "Claude Code 使用相同的工具调用循环架构（发送、tool_use、执行、追加、重复），但 OpenCode 的特色在于使用异步 Effect/Stream 原语进行 LLM 调用，并采用正式的 ToolID schema 注册表。Claude Code 使用扁平的工具列表，而 OpenCode 按类别组织工具（read、write、shell、skill），并配备结构化的输入/输出模式。",
    },
  },
...
while True:
    ...
    learnSections: [
      {
        title: "The Problem: How to Add New Tools Without Changing the Loop",
        content:
      ...
      {
        title: "The Open/Closed Principle in Practice",
        content:
      ...
      {
        title: "Real Implementation: Two-Tier Registration",
        content:
    ...
    designHighlights: [
      { en: "The dispatch map uses `dict.get(name)` with a fallback to `None` rather than `dict[name]`, which would raise a KeyError. This means unknown tool names produce a friendly error message instead of crashing the agent.", zh: "调度表使用 `dict.get(name)` 并回退到 `None`，而非使用会引发 KeyError 的 `dict[name]`。这意味着未知的工具名称会产生友好的错误消息，而不是让 agent 崩溃。" },
  ...
  <line x1="250" y1="306" x2="250" y2="315" stroke="#666" stroke-width="1.5" marker-end="url(#arrow-s02)"/>
    ...
    annotatedCode: {
      ...
      annotations: [
        ...
        { lineStart: 8, lineEnd: 12, text: "The lookup and execution. `TOOL_MAP.get(name)` returns None for unknown tools instead of raising a KeyError. `handler(**input)` unpacks the tool input dictionary as keyword arguments. Each handler defines its own parameter shape.", zhText: "查找与执行。`TOOL_MAP.get(name)` 对未知工具返回 None，而非引发 KeyError。`handler(**input)` 将工具输入字典解包为关键字参数，每个处理函数定义自己的参数形式。" },
        { lineStart: 14, lineEnd: 18, text: "The loop body never changes. It calls execute_tool() and neither knows nor cares which tool ran. This embodies the Open/Closed Principle: the loop is closed to modification but open to extension.", zhText: "循环体从不改变。它调用 execute_tool()，既不知道也不关心是哪个工具在运行。这体现了开闭原则：循环对修改关闭，对扩展开放。" },
      ],
      flowSummary: {
        en: "Flow: Loop → execute_tool(name, input) → TOOL_MAP.get(name) → handler(**input) → result string → appended to messages",
        zh: "流程：循环 → execute_tool(name, input) → TOOL_MAP.get(name) → handler(**input) → 结果字符串 → 追加到消息",
      },
    },
    claudeCodeComparison: {
      ...
      zh: "两个系统都使用调度表和注册表模式。Claude Code 提供内置工具集，并通过 MCP 服务器实现可扩展性。OpenCode 的 TOOL_MAP 在概念上类似，但设计为头等可扩展性：工具注册表是一个正式的数据结构（包含 ToolID、schema 和 handler），而非隐式的 if-else 链。新工具通过元数据注册自身，从而支持通配符权限规则和类型化参数验证等功能。",
    },
  },
  ...
  return handler(input);
    ...
    learnSections: [
      {
        title: "The Problem: Automation Without Recklessness",
        content:
    ...
    designHighlights: [
      { en: "The permission check is async, using await promptUser, which means the agent yields control to the user interface. The agent literally waits for human input before proceeding, with no silent approvals.", zh: "权限检查是异步的（使用 await promptUser），这意味着 agent 将控制权交给用户界面。Agent 在继续之前会等待人工输入，不存在静默批准的情况。" },
  ...
  <text x="390" y="240" fill="#10b981" font-size="10" font-family="system-ui">Yes → Execute</text>
    ...
    annotatedCode: {
      overview: {
        ...
        zh: "权限门以安全检查包装工具执行过程。classifyAction() 确定危险级别，仅危险操作才触发用户提示。安全操作对用户保持透明。",
      },
      annotations: [
        { lineStart: 2, lineEnd: 2, text: "classifyAction() examines both the tool name and the input parameters. Reading a file is safe; writing to system directories is dangerous. This cross-check prevents the agent from misrepresenting the nature of its action.", zhText: "classifyAction() 同时检查工具名称和输入参数。读取文件安全，写入系统目录危险。这种交叉验证防止 agent 对操作性质进行误导性描述。" },
        { lineStart: 3, lineEnd: 7, text: "The async prompt shows the user exactly what tool and parameters are requested. The user sees the actual command, not a sanitized description. This transparency is critical for building trust, as the user always knows what the agent is about to do.", zhText: "异步提示向用户显示所请求的确切工具和参数。用户看到的是实际命令，而非经过修饰的描述。这种透明度对建立信任至关重要，用户始终清楚 agent 将要执行什么操作。" },
        { lineStart: 8, lineEnd: 9, text: "If the user cancels, the agent receives a clear 'cancelled' message. The agent can then try a different approach or ask for clarification. It does not crash or retry the same dangerous action.", zhText: "如果用户取消操作，agent 会收到清晰的 'cancelled' 消息。Agent 随后可以尝试不同的方法或请求澄清，而不会崩溃或重试相同的危险操作。" },
        { lineStart: 10, lineEnd: 12, text: "Safe operations skip the permission gate entirely. The handler is called directly with no user-facing prompt. This keeps the agent fast for routine operations while remaining safe for dangerous ones.", zhText: "安全操作完全跳过权限门，直接调用处理函数而无需用户提示。这使得 agent 在日常操作中保持高效，同时在危险操作上确保安全。" },
      ],
      flowSummary: {
        en: "Flow: executeTool() → classifyAction() → dangerous? → No → execute handler | Yes → promptUser() → approved? → Yes → execute | No → return cancelled",
        zh: "流程：executeTool() → classifyAction() → 危险? → 否 → 执行处理函数 | 是 → promptUser() → 批准? → 是 → 执行 | 否 → 返回已取消",
      },
    },
    claudeCodeComparison: {
      en: "Both prioritize permission gates over sandbox isolation. Claude Code requires explicit user approval for file writes and shell commands. OpenCode's permission system takes this further with classifyAction(), a structured classification function that uses wildcard path patterns (allow, deny, ask) rather than simple binary prompts. The Deferred<PermissionEval> pattern means permissions can be pre-evaluated and cached, reducing user prompt fatigue.",
      zh: "两者都优先使用权限门而非沙箱隔离。Claude Code 要求文件写入和 shell 命令必须获得用户明确批准。OpenCode 的权限系统则更进一步，通过 classifyAction() 这一使用通配符路径模式（allow、deny、ask）而非简单二元提示的结构化分类函数来实现。Deferred<PermissionEval> 模式意味着权限可以预先评估和缓存，从而减少用户提示疲劳。",
    },
  },
  s04: {
    description:
...
for (const hook of registeredHooks["tool.execute.before"]) {
  await hook({ tool, input });
    ...
    learnSections: [
      {
        title: "The Problem: Cross-Cutting Concerns Without Core Pollution",
        content:
      ...
      {
        title: "Hook Design: Named Events with Typed Payloads",
        content:
  ...
  <text x="250" y="310" text-anchor="middle" fill="#666" font-size="10" font-family="system-ui">Each hook gets typed payload + timeout</text>
    ...
    annotatedCode: {
      overview: {
        en: "Plugins register hook functions for named lifecycle events. The core loop invokes these hooks at the appropriate points without any knowledge of which plugins exist.",
        zh: "插件为已命名的生命周期事件注册钩子函数。核心循环在适当的时机调用这些钩子，无需知晓具体有哪些插件存在。",
      },
      annotations: [
        { lineStart: 5, lineEnd: 11, text: "Pre-execution hook: runs before every tool call. This example logs bash commands, but a security plugin could use the same hook to validate commands. Multiple plugins can coexist on the same hook, each serving a different purpose.", zhText: "执行前钩子：在每个工具调用之前触发。此示例用于记录 bash 命令，安全插件则可用同一钩子验证命令。多个插件可在同一钩子上共存，各自承担不同的职责。" },
        { lineStart: 12, lineEnd: 18, text: "Post-execution hook: runs after every tool call with the result attached. This is where error notifications, metrics collection, and audit logging belong.", zhText: "执行后钩子：在每个工具调用之后触发，并附带执行结果。错误通知、指标收集和审计日志等功能都适合在此处实现。" },
        { lineStart: 19, lineEnd: 25, text: "Config hook: runs when the agent loads configuration. This allows plugins to modify the config before it takes effect, such as adding skill paths or setting defaults.", zhText: "配置钩子：在 agent 加载配置时触发。这允许插件在配置生效前进行修改，例如添加技能路径或设置默认值。" },
        { lineStart: 29, lineEnd: 31, text: "Hook invocation: the core loop iterates over all registered hooks for a given event and calls them in order. If a hook throws an error or times out, the exception is caught and logged. Other hooks continue to run unaffected.", zhText: "钩子调用：核心循环遍历给定事件的所有已注册钩子，并按顺序依次调用。如果某钩子抛出异常或超时，错误会被捕获并记录，其他钩子不受影响继续执行。" },
      ],
      flowSummary: {
        en: "Flow: Plugin registers hooks → Hook registry stores them → Loop reaches event point → Iterates registered hooks → Each hook runs with typed context and timeout → Loop continues unaffected by individual hook failures",
        zh: "流程：插件注册钩子 → 钩子注册表存储 → 循环到达事件点 → 遍历已注册钩子 → 每个钩子携带类型化上下文和超时执行 → 单个钩子失败不影响循环继续",
      },
    },
    claudeCodeComparison: {
      en: "Claude Code does not expose a plugin system in the traditional sense. OpenCode's lifecycle hooks (beforeToolExecute, afterToolExecute, beforeLlmCall, onConfigLoad) are a distinguishing feature. They allow any external code to intercept and augment the core loop without modifying the source. This is closer to VS Code's extension model than anything found in Claude Code.",
      zh: "Claude Code 不提供传统意义上的插件系统。OpenCode 的生命周期钩子（beforeToolExecute、afterToolExecute、beforeLlmCall、onConfigLoad）是其显著特性。这些钩子允许任何外部代码拦截并增强核心循环的功能，而无需修改源代码。这更接近 VS Code 的扩展模型，而非 Claude Code 中的任何机制。",
    },
  },
  s05: {
    description:
    ...
    zhDescription:
      "任务在执行前按类别分类。每个类别（quick、deep、visual-engineering、artistry 等）都映射到针对该工作类型优化的 agent 配置。模型不感知路由逻辑的存在，只接收对应类别的系统提示和工具集。",
    codeSample: `const CATEGORIES = {
  quick: {
    model: "fast-model",
    temperature: 0.2,
...
function dispatchTask(input) {
    ...
    learnSections: [
      {
        ...
        zhContent:
          "单一的 agent 配置无法对所有任务达到最优。简单的编辑需要速度和低延迟，而深层的架构工作需要强大的推理能力和更丰富的上下文。对所有任务使用相同的模型和工具，既在简单任务上浪费 tokens，又在复杂任务上力不从心。解决方案是分类系统：在执行前对任务分类，再路由到专门配置。每个分类指定自己的模型、温度、工具集和系统提示。调度过程对模型完全透明，它只需接收正确的配置即可执行。",
      },
      {
        title: "Classifier Design: Heuristics Over Model Calls",
        content:
        ...
        zhTitle: "分类器设计：启发式优于模型调用",
        zhContent:
          "classifyInput() 函数采用基于规则的启发式方法，而非调用另一个 LLM 来判断。任务长度、关键词、正在编辑的文件类型，以及是否存在 'refactor' 或 'add test' 等特定模式，共同决定了分类结果。这使分类过程瞬时完成且零成本。分类器本身也是可插拔的：插件可以添加新类别，也可覆盖现有类别的分类规则。",
      },
      {
        title: "Real Implementation: Agent Selection via Plugin System",
        content:
        ...
        zhTitle: "真实实现：通过插件系统的 Agent 选择",
        zhContent:
          "在 opencode 的实际代码库中，agent 的选择依赖插件系统而非路由表。Agent 通过插件钩子注册（build、plan、general、explore、compaction、title、summary），每种 agent 由其模式（primary、subagent、all）和 hidden 布尔值区分。Web 应用 UI 中的 'categories' 概念仅服务于命令面板的组织。核心运行时按模式而非类别选择 agent。任务级别的路由通过 agent 层次结构实现：build agent（默认 primary）根据任务描述将工作分派给 explore 或 oracle 等子代理。",
      },
    ],
    designHighlights: [
      { en: "Temperature varies by category. Quick tasks use higher temperature for more creative and divergent thinking, while deep tasks use lower temperature for focused and precise reasoning. This is an inexpensive yet effective way to steer model behavior.", zh: "温度随类别而变化。快速任务使用较高温度以激发创造性和发散思维，深度任务则使用较低温度以保证专注和精确。这是控制模型行为的一种廉价而有效的手段。" },
  ...
  <text x="250" y="330" text-anchor="middle" fill="#666" font-size="10" font-family="system-ui">Each → agentLoop(input, config)</text>
    ...
    annotatedCode: {
      overview: {
        en: "The category dispatch system separates 'what to do' from 'how to do it'. classifyInput() analyzes the task and routes it to the appropriate agent configuration.",
        zh: "分类调度系统将'做什么'与'怎么做'相分离。classifyInput() 分析任务后，将其路由到合适的 agent 配置。",
      },
      annotations: [
        { lineStart: 1, lineEnd: 16, text: "Categories are configuration objects, not classes or instances. Each one specifies a model, temperature, tool whitelist, and a system prompt that steers the agent toward the appropriate behavior for that category.", zhText: "类别是配置对象，而非类或实例。每个类别指定了模型、温度、工具白名单和系统提示，引导 agent 展现出适合该类别的工作行为。" },
        { lineStart: 21, lineEnd: 23, text: "classifyInput() analyzes the task text and context using heuristics. Short tasks that reference file paths receive the 'quick' category. Tasks mentioning 'architecture' or spanning many files are classified as 'deep'. UI-related tasks are routed to 'visual-engineering'.", zhText: "classifyInput() 使用启发式规则分析任务文本和上下文。简短且引用文件路径的任务归类为 'quick'；提及 'architecture' 或涉及多个文件的任务归类为 'deep'；与 UI 相关的任务则路由到 'visual-engineering'。" },
        { lineStart: 24, lineEnd: 25, text: "The agentLoop() function is the same one introduced in s01. It is called with the task input and the category's configuration. The loop itself is reused; only the configuration changes.", zhText: "agentLoop() 函数与 s01 中介绍的完全相同。它以任务输入和类别配置为参数被调用。循环本身被复用，变化的只有配置。" },
      ],
      flowSummary: {
        en: "Flow: Task arrives → classifyInput() determines category → CATEGORIES[category] lookup → agentLoop(input, config) runs with category-specific model, tools, and prompt",
        zh: "流程：任务到达 → classifyInput() 确定类别 → 查找 CATEGORIES[category] → agentLoop(input, config) 使用类别专属的模型、工具和提示执行",
      },
    },
    claudeCodeComparison: {
      ...
      zh: "Claude Code 在内部将任务路由到合适的配置，但不暴露路由机制。OpenCode 的 CATEGORIES 映射是一个显式且可配置的路由表。它通过输入分析对任务进行分类，并按类别选择模型、工具集和系统提示（视觉类任务分配视觉模型，编码类任务分配代码模型，依此类推）。这种配置驱动的方式使 agent 能够适应不同的任务领域，而无需更改代码。",
    },
  },
  s06: {
    description:
    ...
    zhDescription:
      "OpenCode 的子代理系统提供 10 个命名代理，每个都有明确的专长文档。Oracle 负责架构分析，explore 搜索代码库，librarian 查阅文档，依此类推。子代理拥有隔离的上下文和干净的消息历史，可通过 @mention 或编程方式调度。",
    codeSample: `const AGENTS = {
  oracle: {
...
async function callSubagent(name, task) {
  ...
  return agentLoop(messages, agent);
    ...
    learnSections: [
      {
        title: "The Problem: Context Pollution in Single-Agent Systems",
        content:
        ...
        zhTitle: "问题：单代理系统中的上下文污染",
        zhContent:
          "在单代理系统中，每个工具的执行结果、错误消息和中间思考都会污染同一个对话空间。Agent 必须持续从不断增长的噪音中重新筛选相关信息。子代理通过创建隔离的上下文来解决这一问题。每个子代理都是一个拥有独立消息历史的 agent 循环。父级调度一个任务后，子代理独立完成工作，只返回最终结果。这防止了父级的上下文被子代理所需的探索过程所稀释。",
      },
      {
        title: "Composition Through Isolation",
        content:
        ...
        zhTitle: "通过隔离实现组合",
        zhContent:
          "每个子代理都运行自己的 agentLoop()，即 s01 中介绍的那个函数。这意味着子代理继承了所有能力（工具、钩子、权限），无需编写任何特殊代码。唯一的区别在于消息历史从空白开始，系统提示定制了具体行为。父级可以并行调度多个子代理（详见 s09），并在它们完成后收集结果。这将 agent 转变为一个编排器：分解复杂任务，将各个部分委托给专家代理，最后综合所有结果。",
      },
      {
        title: "Real Implementation: Task Tool and BackgroundJob",
        content:
        ...
        zhTitle: "真实实现：Task Tool 和 BackgroundJob",
        zhContent:
          "OpenCode 实际的子代理调度使用 task tool (packages/opencode/src/tool/task.ts) 结合 BackgroundJob (packages/core/src/background-job.ts)。与直接函数调用不同，task tool 创建一个带有独立权限范围的子会话。该范围继承父级的 deny 规则，并叠加子代理自身的规则集。子代理支持两种运行模式。在前台模式下，父级通过 background.wait() 等待结果。在后台模式下，调用立即返回，子代理完成后注入结果。task_id 参数支持跨多次调用恢复同一子代理会话，这与简化版本每次调用都创建新上下文的方式不同。",
      },
    ],
    designHighlights: [
      { en: "Subagents are dispatched with a clean context, but the parent can pass relevant context snippets. The prompt building function extracts only the portions of the conversation that are relevant to the subagent's specialty.", zh: "子代理以干净上下文被调度，但父级可传递相关的上下文片段。提示构建函数仅提取对话中与子代理专长相关的部分。" },
  ...
  <text x="250" y="340" text-anchor="middle" fill="#666" font-size="10" font-family="system-ui">Each subagent = fresh agentLoop() + clean message history</text>
    ...
    annotatedCode: {
      overview: {
        ...
        zh: "子代理是带有干净上下文的命名 agent 配置。callSubagent() 创建一个全新的 agent 循环，配有定制化的系统提示和隔离的消息历史。",
      },
      annotations: [
        { lineStart: 1, lineEnd: 18, text: "Agent definitions: named configurations that include a description, model selection, temperature, and tool whitelist. Each agent is optimized for a specific domain of work.", zhText: "Agent 定义：包含描述、模型选择、温度和工具白名单的命名配置。每个 agent 均针对特定工作领域进行了优化。" },
        { lineStart: 20, lineEnd: 22, text: "The callSubagent() function looks up the agent configuration and validates the agent name. Unknown agent names produce a clear error rather than failing silently.", zhText: "callSubagent() 函数查找 agent 配置并验证名称是否有效。未知的 agent 名称会产生明确的错误信息，而非静默失败。" },
        { lineStart: 24, lineEnd: 28, text: "Building a clean context: a fresh system message with the agent's prompt, combined with the user's task. No prior tool results, no conversation history. Only the task at hand.", zhText: "构建干净上下文：一条包含 agent 提示的新系统消息，加上用户的任务。没有之前的工具结果，没有对话历史。只有当前的任务。" },
        { lineStart: 30, lineEnd: 31, text: "The same agentLoop() from s01 is reused. Subagents are not special. They are the same loop running with different configuration. This is composition over inheritance in practice.", zhText: "复用 s01 中同一个 agentLoop()。子代理并无特殊之处，它们是以不同配置运行的同一循环。这正是组合优于继承的实践体现。" },
      ],
      flowSummary: {
        en: "Flow: Parent identifies subtask → callSubagent(name, task) → buildSystemPrompt(agent) → fresh agentLoop() starts → subagent works independently → returns structured result → parent synthesizes all results",
        zh: "流程：父级识别子任务 → callSubagent(name, task) → buildSystemPrompt(agent) → 启动全新 agentLoop() → 子代理独立执行 → 返回结构化结果 → 父级综合所有结果",
      },
    },
    claudeCodeComparison: {
      ...
      zh: "两者都支持子代理委派。Claude Code 通过其 agent 模式实现，而 OpenCode 则使用其子代理系统。OpenCode 的方法粒度更细：每个子代理拥有自己的循环、工具集和权限上下文。OpenCode 中的 Explorer、Librarian 和 Oracle 等专用代理类型，对应了真实的工程角色。每种代理都配有经过调优的系统提示和工具访问权限。父代理可以并行向多个子代理分派任务，并综合它们的结果。",
    },
  },
  s07: {
    description:
    ...
    codeSample: `# Example SKILL.md file:
# ---
# name: react-testing
...
def load_skill(name):
    ...
    learnSections: [
      {
        title: "The Problem: How to Inject Specialized Knowledge Without Overloading Context",
        content:
        ...
        zhTitle: "问题：如何在不过载上下文的情况下注入专业知识",
        zhContent:
      ...
      {
        title: "Priority-Based Loading: Predictable Override",
        content:
    ...
    designHighlights: [
      { en: "Skill descriptions serve double duty: the agent uses them to discover relevant skills at runtime. When encountering an unfamiliar task, the agent scans available descriptions and requests the matching skill. This is a form of just-in-time knowledge retrieval.", zh: "技能描述承担双重职责：agent 在运行时利用它们发现相关技能。遇到不熟悉的任务时，agent 扫描可用描述并请求匹配的技能。这是一种即时知识检索形式。" },
      { en: "Skills are pure markdown with no executable code. This guarantees safe loading from untrusted repositories. A skill file cannot execute arbitrary code or modify agent behavior beyond providing textual context.", zh: "技能是纯 markdown，不含可执行代码。这保证了从未知仓库加载的安全性。技能文件无法执行任意代码，也无法在提供文本上下文之外修改 agent 的行为。" },
    ],
    flowchartSvg: `<svg viewBox="0 0 500 320" xmlns="http://www.w3.org/2000/svg" style="max-width:100%;height:auto">
  <defs>
  ...
  <text x="250" y="318" text-anchor="middle" fill="#666" font-size="10" font-family="system-ui">③ Built-in: fallback if not found in project or personal</text>
    ...
    annotatedCode: {
      overview: {
        en: "Skill loading follows a simple priority-chain pattern. Directories are checked in order, returning the first match. This makes override behavior fully predictable.",
        zh: "技能加载遵循简单的优先级链模式。按顺序检查目录，返回第一个匹配项。这使覆盖行为完全可预测。",
      },
      annotations: [
        { lineStart: 2, lineEnd: 5, text: "YAML frontmatter defines the skill's name and description. The description serves as the discovery index: the agent scans these at runtime to find relevant skills.", zhText: "YAML 前置元数据定义了技能的名称和描述。描述充当发现索引：agent 在运行时扫描这些内容以查找相关技能。" },
        { lineStart: 8, lineEnd: 11, text: "Priority ordering is hardcoded: project first, then personal config, then built-in. Placing a file with the same name at a higher-priority location overrides any lower-tier skill.", zhText: "优先级顺序是硬编码的：项目优先，然后是个人配置，最后是内置。在更高级别位置放置同名文件即可覆盖任何低层级技能。" },
        { lineStart: 13, lineEnd: 17, text: "The load_skill() function iterates each priority directory in order. It returns the first file found. If no file exists across all directories, it returns None: the skill does not exist.", zhText: "load_skill() 函数按顺序遍历每个优先级目录。返回第一个找到的文件。如果所有目录中都不存在该文件，则返回 None，该技能不存在。" },
      ],
      flowSummary: {
        en: "Flow: load_skill(name) → check project/ → found? → return | not found → check personal/ → found? → return | not found → check built-in/ → found? → return | not found → None",
        zh: "流程：load_skill(name) → 检查 project/ → 找到? → 返回 | 未找到 → 检查 personal/ → 找到? → 返回 | 未找到 → 检查 built-in/ → 找到? → 返回 | 未找到 → None",
      },
    },
    claudeCodeComparison: {
      ...
      zh: "两者都支持基于 SKILL.md 的技能加载。OpenCode 的三层优先级链（项目级、用户级、内置级）比 Claude Code 的隐式发现机制更明确。优先级系统意味着项目技能覆盖个人技能，个人技能覆盖内置默认值。这对团队环境至关重要，项目级标准必须具有优先权。",
    },
  },
  s08: {
    description:
...
export default function myPlugin() {
  return {
    ...
    learnSections: [
      {
        title: "The Problem: Stable Core, Unstable Extensions",
        content:
      ...
      {
        title: "Comprehensive Hook Coverage: Why Every Hook Exists",
        content:
    ...
    designHighlights: [
      { en: "The Hooks interface uses optional methods: a plugin implements only the hooks it needs. No base class to extend, no abstract methods to implement. This is structural typing in action.", zh: "Hooks 接口使用可选方法：插件只实现它需要的钩子。无需继承基类或实现抽象方法。这是结构化类型系统的实际应用。" },
  ...
  <text x="250" y="340" text-anchor="middle" fill="#666" font-size="10" font-family="system-ui">Plugins register hook functions → Core calls them at lifecycle points → Never modified</text>
    ...
    annotatedCode: {
      overview: {
        en: "The Hooks interface is the complete public API for extensions. Every method is optional: a plugin implements only what it needs. The core calls registered hooks at lifecycle points without knowing which plugin registered them.",
        zh: "Hooks 接口是扩展的完整公共 API。每个方法都是可选的：插件只实现其所需的部分。核心在生命周期点调用已注册的钩子，而无需知道是哪个插件注册的。",
      },
      annotations: [
        ...
        { lineStart: 7, lineEnd: 8, text: "The permission hook allows plugins to override the default permission gate with custom logic. An enterprise plugin might enforce IP-based restrictions or require multi-party approval for destructive operations.", zhText: "权限钩子允许插件用自定义逻辑覆盖默认权限门。企业插件可以强制执行基于 IP 的限制或要求对破坏性操作进行多方审批。" },
        { lineStart: 14, lineEnd: 24, text: "A real plugin factory function. It returns an object containing only the hooks it needs. The config hook modifies settings; the tool.execute.before hook logs every tool invocation. No base class, no interface implementation: just an object that matches the expected shape.", zhText: "一个真实的插件工厂函数。它返回一个只包含所需钩子的对象。config 钩子修改设置；tool.execute.before 钩子记录每次工具调用。没有基类，无需接口实现，只返回一个匹配预期形状的对象。" },
      ],
      flowSummary: {
        en: "Flow: Plugin → returns hook object → PluginRegistry.register(plugin) → Core reaches lifecycle event → iterates registered hooks[event] → calls each with typed context → collects results or mutations",
        zh: "流程：插件 → 返回钩子对象 → PluginRegistry.register(plugin) → 核心到达生命周期事件 → 遍历已注册的 hooks[event] → 用类型化上下文调用每个钩子 → 收集结果或变更",
      },
    },
    claudeCodeComparison: {
      ...
      zh: "Plugin SDK 是 OpenCode 最独特的特性之一，Claude Code 没有对等功能。registerPlugin(plugin: Plugin) → hooks[] → lifecycle events 模式支持市场风格的生态系统，第三方插件可以添加新工具、权限策略和配置转换，而无需 fork 核心仓库。",
    },
  },
  s09: {
    description:
    ...
    zhDescription:
      "慢操作在后台线程中执行，使 agent 可以继续推理。结果被推送到事件总线，无需轮询。这大幅提升了长时间运行的测试、网络调用、文件下载等操作的吞吐量。",
...
const taskId = await runBackground("build", () =>
  exec("npm run build")
    ...
    learnSections: [
      {
        title: "The Problem: Blocking Operations Kill Agent Throughput",
        content:
        ...
        zhTitle: "问题：阻塞操作扼杀 Agent 吞吐量",
        zhContent:
          "当 agent 运行慢命令（如 npm install、测试套件或 git clone）时，循环被阻塞。模型处于空闲状态，等待结果。30 秒的测试运行意味着 30 秒的闲置。后台任务通过将执行与推理解耦来解决这个问题。Agent 启动任务，继续推理（规划后续步骤、准备后续工作），并在结果通过事件总线到达时获取。这将串行的等待时间转化为并行的生产能力。",
      },
      {
        title: "Event Bus: The Glue Between Parallel Workstreams",
        content:
        ...
        zhTitle: "事件总线：并行工作流之间的粘合剂",
        zhContent:
          "事件总线是一种简单的发布-订阅机制。后台任务在完成时发出事件；agent 循环监听这些事件并将结果合并到对话中。这种模式比回调更灵活（多个监听器可以响应同一事件），比轮询更高效（无需浪费周期检查任务状态）。事件总线还解决了合并问题：如果 agent 调度了多个后台任务，它可以等待所有任务完成，或响应最先完成的那一个。",
      },
      {
        title: "Real Implementation: Promotion Pattern",
        content:
        ...
        zhTitle: "真实实现：Promotion 模式",
        zhContent:
          "OpenCode 实际的 BackgroundJob 系统使用 promotion 模式而非简单的启动等待方式。任务经历一系列状态转换：start、extend（链式顺序工作）、promote（标记为后台）、wait for promotion。关键在于任务可以以前台模式启动，随后提升为后台，反之亦然。这实现了先前台后决定的模式：agent 同步启动任务，如果耗时过长则将其提升为后台。子代理 task tool 正是使用这种模式：前台子代理等待结果，后台子代理独立继续。",
      },
    ],
    designHighlights: [
  ...
  <text x="270" y="300" fill="#666" font-size="9" font-family="system-ui">Agent notices result on next LLM call</text>
    ...
    annotatedCode: {
      overview: {
        en: "The background task system lets the agent delegate slow operations to a separate thread and continue reasoning. Results arrive through an event bus, not via polling.",
        zh: "后台任务系统允许 agent 将慢操作委托给独立线程并继续推理。结果通过事件总线到达，而非轮询。",
      },
      annotations: [
        ...
        { lineStart: 4, lineEnd: 10, text: "runBackground() starts the task and returns immediately. The promise's .then() handler emits a completion event. The result is not forced into the conversation; the loop picks it up when ready.", zhText: "runBackground() 启动任务并立即返回。Promise 的 .then() 处理函数发出完成事件。结果不会被强制注入对话；循环在就绪时获取它。" },
        { lineStart: 12, lineEnd: 17, text: "The event listener pushes completed results into the message history as user messages. The agent perceives them as though the user provided the information, consistent with how tool results flow through the core loop.", zhText: "事件监听器将完成的结果作为用户消息推送到消息历史中。Agent 感知这些结果如同用户提供的信息，与核心循环中工具结果的处理方式一致。" },
        { lineStart: 19, lineEnd: 20, text: "The agent starts a build task and immediately continues reasoning about what to do next. The build result arrives later through the event bus.", zhText: "Agent 启动构建任务并立即继续推理后续步骤。构建结果稍后通过事件总线到达。" },
      ],
      flowSummary: {
        en: "Flow: Agent identifies slow operation → runBackground(id, fn) → fn runs in background thread → Agent continues reasoning → fn completes → Event Bus emits 'task.complete' → Listener pushes result to messages → Agent sees result on next iteration",
        zh: "流程：Agent 识别慢操作 → runBackground(id, fn) → fn 在后台线程运行 → Agent 继续推理 → fn 完成 → 事件总线发出 'task.complete' → 监听器将结果推送到消息 → Agent 在下一次迭代中看到结果",
      },
    },
    claudeCodeComparison: {
      en: "Claude Code processes tasks synchronously: each tool call blocks the agent loop. OpenCode's background job system is a clear differentiator. Long-running operations (web searches, code compilation, test suites) run in the background while the agent continues reasoning. Results arrive through the event bus and are injected into the conversation when ready, enabling a non-blocking, concurrent workflow.",
      zh: "Claude Code 同步处理任务：每个工具调用阻塞 agent 循环。OpenCode 的后台任务系统是一个明显的差异化优势。长时间运行的操作（网络搜索、代码编译、测试套件）在后台运行，同时 agent 继续推理。结果通过事件总线到达并在准备好时注入对话，实现非阻塞、并发的工作流。",
    },
  },
  s10: {
    description:
    ...
    zhDescription:
      "OpenCode 使用单个配置文件声明模型、插件、技能路径和所有其他设置。默认配置文件位于 ~/.config/opencode/opencode.jsonc。项目可以通过在根目录放置自己的 opencode.jsonc 来覆盖设置；配置会合并而非替换。",
    codeSample: `// Global config: ~/.config/opencode/opencode.jsonc
{
...
function loadConfig() {
  ...
  return global;
    ...
    learnSections: [
      {
        title: "The Problem: Every Project Is Different, Every Developer Has Preferences",
        content:
        ...
        zhTitle: "问题：每个项目都不同，每个开发者都有偏好",
        zhContent:
          "开发者可能在大多数任务中偏好 Claude Sonnet，但特定项目需要 GPT-4。他们可能有个人技能集合，而团队维护共享项目技能。配置系统必须协调全局默认值与项目级覆盖，同时保留覆盖任何内容而不复制所有内容的能力。深度合并解决了这个问题：全局配置提供默认值；项目配置覆盖特定字段；其余内容保持不变。",
      },
      {
        title: "Discoverability and the \$schema",
        content:
        ...
        zhTitle: "可发现性与 \$schema",
        zhContent:
          "opencode.jsonc 文件通过 $schema URL 引用 JSON Schema。这并非装饰性的：它在编辑配置文件时启用 IDE 自动完成、验证和内联文档。该 schema 还充当所有可用配置选项的规范参考。新选项首先添加到 schema，然后在代码中实现。这种文档驱动的方法确保配置面始终有文档和验证。",
      },
      {
        title: "Real Implementation: Event-Sourced Sessions with SQLite",
        content:
        ...
        zhTitle: "真实实现：带 SQLite 的事件溯源会话",
        zhContent:
          "OpenCode 的会话系统超越了配置管理。会话是事件溯源的，通过 Drizzle ORM 存储在 SQLite 中。每个会话维护一个持久事件的 Timeline。这意味着会话在重启后仍然存在，支持通过事件回放进行回滚，并且可以通过同一 SQLite 存储在进程之间共享。SessionRunCoordinator 按会话序列化执行，带有唤醒合并：如果两个提示同时到达，它们被排队并顺序处理，而不是并行处理。",
      },
    ],
    designHighlights: [
      { en: "Config discovery walks up the directory tree from the project root, not from the current working directory. This means any subdirectory within a project correctly resolves to the project's opencode.jsonc.", zh: "配置发现从项目根目录向上遍历目录树，而非从当前工作目录开始。这意味着项目中的任何子目录都能正确解析到项目的 opencode.jsonc。" },
      { en: "JSONC format (JSON with comments) is used instead of plain JSON. Comments make the config self-documenting: developers can annotate their choices inline without maintaining a separate documentation file.", zh: "使用 JSONC 格式（带注释的 JSON）而非纯 JSON。注释使配置自文档化：开发者可以在行内注解自己的选择，无需维护单独的文档文件。" },
    ],
    flowchartSvg: `<svg viewBox="0 0 500 320" xmlns="http://www.w3.org/2000/svg" style="max-width:100%;height:auto">
  <defs>
  ...
  <line x1="250" y1="245" x2="250" y2="268" stroke="#10b981" stroke-width="2" marker-end="url(#arrow-s10)"/>
    ...
    annotatedCode: {
      overview: {
        ...
        zh: "配置系统使用两层合并模式：全局默认值加上项目覆盖产生有效配置。deepMerge() 处理嵌套对象，使部分覆盖能够直观地工作。",
      },
      annotations: [
        { lineStart: 2, lineEnd: 7, text: "Global config: the defaults that work for most projects. Located at ~/.config/opencode/, it serves as the single source of truth for the user's agent preferences.", zhText: "全局配置：适用于大多数项目的默认值。位于 ~/.config/opencode/，是用户 agent 偏好的单一真相来源。" },
        { lineStart: 10, lineEnd: 14, text: "Per-project override: specifies only what differs. The model is different, and a project-specific plugin is added. Everything else, including skill paths, is inherited from the global config.", zhText: "项目级覆盖：只指定不同的内容。模型不同，并添加了项目特定的插件。其他所有内容，包括技能路径，都从全局配置继承。" },
        { lineStart: 16, lineEnd: 22, text: "The loadConfig() function loads the global config, walks up the directory tree looking for a project config, deep merges if found, or uses the global config as-is. The merge is deep: nested objects are merged recursively, not replaced outright.", zhText: "loadConfig() 函数加载全局配置，向上遍历目录树查找项目配置，如果找到则深度合并，否则按原样使用全局配置。合并是深度的：嵌套对象被递归合并，而非直接替换。" },
      ],
      flowSummary: {
        en: "Flow: loadConfig() → read global (~/.config/opencode/opencode.jsonc) → findProjectConfig() → found? → deepMerge(global, local) → effective config | not found → global config → agent starts",
        zh: "流程：loadConfig() → 读取全局配置 → findProjectConfig() → 找到? → deepMerge(全局, 本地) → 有效配置 | 未找到 → 全局配置 → agent 启动",
      },
    },
    claudeCodeComparison: {
      ...
      zh: "两者都使用配置文件，但 OpenCode 的多源 deepMerge 模式更精细。全局配置 (~/.config/opencode/) 提供默认值，项目配置覆盖特定内容，两者被递归合并。Claude Code 使用更简单的单一配置方式。OpenCode 的会话系统还增加了压缩和消息版本管理，这对于长时间运行的会话至关重要，否则会超出上下文窗口。",
    },
  },
};
